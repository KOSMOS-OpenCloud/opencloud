...
type Config struct {
	...
	CrossSpaceMove             bool   `yaml:"cross_space_move" env:"GATEWAY_CROSS_SPACE_MOVE" desc:"Enable server-side cross-space move with metadata preservation. When enabled, moving files between spaces preserves custom metadata (xattr)." introductionVersion:"7.2.0"`
